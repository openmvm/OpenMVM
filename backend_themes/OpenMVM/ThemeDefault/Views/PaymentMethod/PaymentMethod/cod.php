...
<div id="content-wrapper" class="content-wrapper min-vh-100">
	...
	<section class="heading-container bg-dark p-3">
		<h2 class="heading-title text-white"><i class="fas fa-hand-holding-usd fa-fw"></i> <?php echo $heading_title; ?></h2>
		  ...
		  <div class="accordion-item">
		    <h2 class="accordion-header" id="headingAdministratorGroup">
		      <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseAdministratorGroup" aria-expanded="true" aria-controls="collapseAdministratorGroup">
		        <i class="fas fa-hand-holding-usd fa-fw"></i>&nbsp;&nbsp;<?php echo $heading_title; ?>
		      </button>
		    </h2>
		          ...
		          <div class="tab-pane show active" id="general" role="tabpanel" aria-labelledby="general-tab">
		            <fieldset>
							  	<div class="form-floating mb-3">
									  <input type="number" name="payment_cod_total" value="<?php echo $payment_cod_total; ?>" class="form-control" id="input-total" placeholder="<?php echo lang('Entry.entry_total', array(), $lang->getBackEndLocale()); ?>">
									  <label for="input-total"><?php echo lang('Entry.entry_total', array(), $lang->getBackEndLocale()); ?></label>
									</div>
									<div class="form-floating mb-3">
									  <select name="payment_cod_order_status_id" class="form-select" id="input-order-status-id" aria-label="order-status-id">
									  	<?php foreach ($order_statuses as $order_status) { ?>
			                  <?php if ($order_status['order_status_id'] == $payment_cod_order_status_id) { ?>
			                  ...
			                  <?php } ?>
									...
									<div class="form-floating mb-3">
									  <select name="payment_cod_geo_zone_id" class="form-select" id="input-geo-zone-id" aria-label="geo-zone-id">
									  	<?php foreach ($geo_zones as $geo_zone) { ?>
			                  <?php if ($geo_zone['geo_zone_id'] == $payment_cod_geo_zone_id) { ?>
			                  ...
			                  <?php } ?>
							  	...
							  	<div class="form-floating mb-3">
									  <input type="number" name="payment_cod_sort_order" value="<?php echo $payment_cod_sort_order; ?>" class="form-control" id="input-sort-order" placeholder="<?php echo lang('Entry.entry_sort_order', array(), $lang->getBackEndLocale()); ?>">
									  <label for="input-sort-order"><?php echo lang('Entry.entry_sort_order', array(), $lang->getBackEndLocale()); ?></label>
									</div>
									<div class="form-floating mb-3">
									  <select name="payment_cod_status" class="form-select" id="input-status" aria-label="status">
		                  <?php if ($payment_cod_status) { ?>
