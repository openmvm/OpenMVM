...
<div id="content-wrapper" class="content-wrapper min-vh-100">
	...
	<section class="heading-container bg-dark p-3">
		<h2 class="heading-title text-white"><i class="fas fa-university fa-fw"></i> <?php echo $heading_title; ?></h2>
  ...
  <!-- /.heading-container -->
    ...
    <div class="clearfix mb-3">
	    <div class="float-end">
	      ...
	      <a href="<?php echo base_url($_SERVER['app.adminDir'] . '/payment_methods/' . $administrator_token); ?>" class="btn btn-secondary" data-bs-toggle="tooltip" data-bs-placement="top" title="<?php echo lang('Button.button_cancel', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-long-arrow-alt-left"></i></a>
	    </div>
    </div>
		  ...
		  <div class="accordion-item">
		    <h2 class="accordion-header" id="headingAdministratorGroup">
		      <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseAdministratorGroup" aria-expanded="true" aria-controls="collapseAdministratorGroup">
		        <i class="fas fa-university fa-fw"></i>&nbsp;&nbsp;<?php echo $heading_title; ?>
		      </button>
		    </h2>
		          ...
		          <div class="tab-pane show active" id="general" role="tabpanel" aria-labelledby="general-tab">
		            <fieldset>
							  	...
							  	<div class="form-floating mb-3">
									  <textarea style="height: 200px" name="payment_bank_transfer_bank<?php echo $language['language_id']; ?>" class="form-control<?php if ($validation->hasError('payment_bank_transfer_bank' . $language['language_id'])) { ?> is-invalid<?php } ?>" id="input-bank" placeholder="<?php echo lang('Entry.entry_bank', array(), $lang->getBackEndLocale()); ?>"><?php echo $payment_bank_transfer_bank[$language['language_id']]; ?></textarea>
									  <label for="input-bank"><img src="<?php echo base_url('/assets/flags/' . $language['image']); ?>" /> <?php echo lang('Entry.entry_bank', array(), $lang->getBackEndLocale()); ?></label>
									  <?php if ($validation->hasError('payment_bank_transfer_bank' . $language['language_id'])) { ?>
	                  <div class="text-danger small"><?php echo $validation->getError('payment_bank_transfer_bank' . $language['language_id']); ?></div>
	                	<?php } ?>
									</div>
									<?php } ?>
							  	<div class="form-floating mb-3">
									  <input type="number" name="payment_bank_transfer_total" value="<?php echo $payment_bank_transfer_total; ?>" class="form-control" id="input-total" placeholder="<?php echo lang('Entry.entry_total', array(), $lang->getBackEndLocale()); ?>">
									  <label for="input-total"><?php echo lang('Entry.entry_total', array(), $lang->getBackEndLocale()); ?></label>
									</div>
									<div class="form-floating mb-3">
									  <select name="payment_bank_transfer_order_status_id" class="form-select" id="input-order-status-id" aria-label="order-status-id">
									  	<?php foreach ($order_statuses as $order_status) { ?>
			                  <?php if ($order_status['order_status_id'] == $payment_bank_transfer_order_status_id) { ?>
			                  ...
			                  <?php } ?>
							  	...
							  	<div class="form-floating mb-3">
									...
									<div class="form-floating mb-3">
									  <select name="payment_bank_transfer_status" class="form-select" id="input-status" aria-label="status">
		                  <?php if ($payment_bank_transfer_status) { ?>
