...
<div id="content-wrapper" class="content-wrapper min-vh-100">
  ...
  <!-- /.heading-container -->
    ...
    <?php echo form_open(base_url($_SERVER['app.adminDir'] . '/frontend_layouts/' . $administrator_token)); ?>
    <div class="clearfix mb-3">
	    <div class="float-end">
	      <a href="<?php echo base_url($_SERVER['app.adminDir'] . '/frontend_layouts/add/' . $administrator_token); ?>" class="btn btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="<?php echo lang('Button.button_add', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-plus"></i></a>
		  ...
		  <div class="accordion-item">
		    <h2 class="accordion-header" id="headingAdministratorGroup">
		      <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseAdministratorGroup" aria-expanded="true" aria-controls="collapseAdministratorGroup">
		        <i class="fas fa-images fa-fw"></i>&nbsp;&nbsp;<?php echo lang('Heading.heading_frontend_layouts', array(), $lang->getBackEndLocale()); ?>
		      </button>
		    </h2>
		      ...
		      <div class="accordion-body">
		        <div class="table-responsive">
		          <table class="table table-hover">
		            <caption><?php echo lang('Caption.caption_list_of_frontend_layouts', array(), $admin_locale); ?></caption>
		            <thead>
		              <tr>
		                <th scope="col"><input type="checkbox" onclick="$('input[name*=\'selected\']').prop('checked', this.checked);" /></th>
		                <th scope="col"><?php echo lang('Column.column_name', array(), $admin_locale); ?></th>
		                <th scope="col"><?php echo lang('Column.column_action', array(), $admin_locale); ?></th>
		              </tr>
		            </thead>
		            <tbody>
		              <?php if ($frontend_layouts) { ?>
		                <?php foreach ($frontend_layouts as $frontend_layout) { ?>
		                <tr>
		                  <th scope="row">
		                    <?php if (in_array($frontend_layout['layout_id'], $selected)) { ?>
		                    <input type="checkbox" name="selected[]" value="<?php echo $frontend_layout['layout_id']; ?>" checked="checked" />
		                    <?php } else { ?>
		                    <input type="checkbox" name="selected[]" value="<?php echo $frontend_layout['layout_id']; ?>" />
		                    <?php } ?>
		                  </th>
		                  <td><?php echo $frontend_layout['name']; ?></td>
		                  <td><a href="<?php echo $frontend_layout['edit']; ?>" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="<?php echo lang('Button.button_edit', array(), $admin_locale); ?>"><i class="fa fa-edit"></i></a></td>
		                </tr>
		                <?php } ?>
		              <?php } else { ?>
		                <tr>
		                  <td colspan="3" class="text-center text-info"><?php echo lang('Error.error_no_data_found', array(), $admin_locale); ?></td>
		                </tr>
		              <?php } ?>
		            </tbody>
		          </table>
