<div id="filemanagerPopUp" class="card text-center min-vh-100">
		...
		<div class="clearfix">
			<div class="float-start">
				<div class="btn-group" role="group" aria-label="Home">
					<span id="button-home" class="btn btn-light btn-sm border border-secondary" title="<?php echo lang('Button.button_home', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-home fa-fw"></i></span>
				</div>
				<div class="btn-group dropdown" role="group" aria-label="Folder">
				  <span id="dropdownFolderButton" class="btn btn-light btn-sm border border-secondary dropdown-toggle" data-bs-toggle="dropdown" title="<?php echo lang('Button.button_new_folder', array(), $lang->getBackEndLocale()); ?>" aria-haspopup="true" aria-expanded="false"><i class="fas fa-folder-plus fa-fw"></i></span>
				  <div class="dropdown-menu p-1" aria-labelledby="dropdownMenuButton">
						<div class="input-group input-group-sm" style="width: 200px;">
						  <input type="text" id="input-new-folder" class="form-control" placeholder="<?php echo lang('Entry.entry_folder_name', array(), $lang->getBackEndLocale()); ?>" aria-label="<?php echo lang('Entry.entry_folder_name', array(), $lang->getBackEndLocale()); ?>" aria-describedby="button-new-folder">
				...
				<div class="btn-group" role="group" aria-label="Upload">
					<span id="button-upload" class="btn btn-light btn-sm border border-secondary" title="<?php echo lang('Button.button_upload', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-upload fa-fw"></i></span>
					<span id="button-download" class="btn btn-light btn-sm border border-secondary" title="<?php echo lang('Button.button_download', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-download fa-fw"></i></span>
				</div>
				<div class="btn-group" role="group" aria-label="Cache">
					<span id="button-clear-cache" class="btn btn-light btn-sm border border-secondary" title="<?php echo lang('Button.button_rebuild_cache', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-broom fa-fw"></i></span>
					<span id="button-refresh" class="btn btn-light btn-sm border border-secondary" data-toggle="tooltip" data-placement="top" title="<?php echo lang('Button.button_refresh', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-sync fa-fw"></i></span>
				</div>
				<div class="btn-group" role="group" aria-label="Delete">
					<span id="button-delete" class="btn btn-light btn-sm border border-secondary" title="<?php echo lang('Button.button_delete', array(), $lang->getBackEndLocale()); ?>"><i class="fas fa-trash-alt fa-fw"></i></span>
				</div>
			</div>
  ...
  <div class="card-body">
		<div id="filemanagerContent" style="min-height: 250px;">
			<div id="spinner" class="text-center d-none"><span><i class="fas fa-spinner fa-spin fa-3x"></i><br /><?php echo lang('Text.text_loading', array(), $lang->getBackEndLocale()); ?></span></div>
		</div> 
  </div>
...
function refresh(path) {
	...
	$.ajax({
		url: '<?php echo base_url($_SERVER['app.adminDir'] . '/filemanager/contents/' . $administrator_token); ?>',
		type: 'post',
		dataType: 'json',
    headers : {
...
$( "#button-new-folder" ).click(function() {
	...
	if (folder_name != null && folder_name != '') {
		$.ajax({
			url: '<?php echo base_url($_SERVER['app.adminDir'] . '/filemanager/create_directory/' . $administrator_token); ?>',
			type: 'post',
			dataType: 'json',
	    headers : {
	      'csrf-token': $('meta[name="csrf-token"]').attr('content')
		...
		alert( '<?php echo lang('Error.error_folder_name', array(), $lang->getBackEndLocale()); ?>' );
	}
    ...
    droppedFiles = e.originalEvent.dataTransfer.files;
	  ...
	  if (droppedFiles) {
		...
		timer = setInterval(function() {
			...
			$.ajax({
				url: '<?php echo base_url($_SERVER['app.adminDir'] . '/filemanager/upload/' . $administrator_token); ?>',
				type: 'post',
				dataType: 'json',
	    	headers : {
		      'csrf-token': $('meta[name="csrf-token"]').attr('content')
...
$('#button-download').on('click', function(e) {
	...
	if (confirm('<?php echo lang('Text.text_confirm', array(), $lang->getBackEndLocale()); ?>')) {
		$.ajax({
			url: '<?php echo base_url($_SERVER['app.adminDir'] . '/filemanager/compress/' . $administrator_token); ?>',
			type: 'post',
			dataType: 'json',
    	headers : {
	      'csrf-token': $('meta[name="csrf-token"]').attr('content')
			...
			success: function(json) {
				if (json['success']) {
					window.location.href = '<?php echo base_url($_SERVER['app.adminDir'] . '/filemanager/download/' . $administrator_token); ?>?archive=' + json['success'];
				} else {
					alert(json['error']);
				}
			},
...
$('#button-clear-cache').on('click', function(e) {
	...
	if (confirm('<?php echo lang('Text.text_confirm', array(), $lang->getBackEndLocale()); ?>')) {
		$.ajax({
			url: '<?php echo base_url($_SERVER['app.adminDir'] . '/filemanager/clear_cache/' . $administrator_token); ?>',
			type: 'post',
			dataType: 'json',
    	headers : {
	      'csrf-token': $('meta[name="csrf-token"]').attr('content')
...
$('#button-delete').on('click', function(e) {
	...
	if (confirm('<?php echo lang('Text.text_confirm', array(), $lang->getBackEndLocale()); ?>')) {
		$.ajax({
			url: '<?php echo base_url($_SERVER['app.adminDir'] . '/filemanager/delete/' . $administrator_token); ?>',
			type: 'post',
			dataType: 'json',
    	headers : {
	      'csrf-token': $('meta[name="csrf-token"]').attr('content')
