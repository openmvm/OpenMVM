...
class Language_Editor extends \App\Controllers\BaseController
{
    ...
    public function index()
    {
        ...
        $data['action'] = $this->url->administratorLink('admin/developer/language_editor');
        ...
        if ($this->request->getMethod() == 'post' && !empty($this->request->getPost('selected'))) {
            if (!$this->administrator->hasPermission('modify', 'Developer/Language_Editor')) {
                ...
                return redirect()->to($this->url->administratorLink('admin/developer/language_editor'));
            }
    ...
    public function edit()
    {
        ...
        $data['action'] = $this->url->administratorLink('admin/developer/language_editor/edit/' . $this->uri->getSegment($this->uri->getTotalSegments()));

        if ($this->request->getMethod() == 'post') {
            if (!$this->administrator->hasPermission('modify', 'Developer/Language_Editor')) {
                $this->session->set('error', lang('Error.modify_permission'));

                return redirect()->to($this->url->administratorLink('admin/developer/language_editor/edit/' . $this->uri->getSegment($this->uri->getTotalSegments())));
            }
            ...
            foreach ($languages as $language) {
                ...
                foreach ($strings as $string) {
                    $content .= '    \'' . $string['key'] . '\'' . ' => ' . '\'' . $string['value'][$language['code']] . '\',' . "\n";
                }
    ...
    public function get_list($data)
    {
        ...
        foreach ($files as $file) {
            ...
            $data['files'][] = [
                ...
                'href' => $this->url->administratorLink('admin/developer/language_editor/edit/' . $pathinfo['filename']),
            ];
        }
        ...
        $data['add'] = $this->url->administratorLink('admin/developer/language_editor/add');
        $data['cancel'] = $this->url->administratorLink('admin/common/dashboard');
		
        if ($this->administrator->hasPermission('access', 'Developer/Language_Editor')) {
            // Header
            $header_params = array(
            ...
            return $this->template->render('ThemeAdmin', 'com_openmvm', 'Basic', 'Developer\language_editor_list', $data);
        } else {
            $data = [];
    ...
    public function get_form($data)
    {
        ...
        $data['cancel'] = $this->url->administratorLink('admin/developer/language_editor');
        ...
        if ($this->administrator->hasPermission('access', 'Developer/Language_Editor')) {
            ...
            return $this->template->render('ThemeAdmin', 'com_openmvm', 'Basic', 'Developer\language_editor_form', $data);
        } else {
            $data = [];
