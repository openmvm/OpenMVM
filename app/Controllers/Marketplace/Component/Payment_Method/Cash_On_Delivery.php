...
namespace App\Controllers\Marketplace\Component\Payment_Method;
...
class Cash_On_Delivery extends \App\Controllers\BaseController
{
    ...
    {
        $data['instruction'] = nl2br($this->setting->get('component_payment_method_cash_on_delivery_instruction_' . $this->language->getCurrentId()));

        if (!empty($seller_id)) {
            $data['confirm'] = $this->url->customerLink('marketplace/component/payment_method/cash_on_delivery/confirm', ['seller_id' => $seller_id], true);
        } else {
    ...
    public function confirm()
    {
        ...
        if ($this->session->has('checkout_payment_method_code') && $this->session->get('checkout_payment_method_code') == 'Cash_On_Delivery') {
            $error = false;
            ...
            if (!$error) {
                ...
                foreach ($sellers as $seller) {
                    ...
                    $order_status_id = $this->setting->get('component_payment_method_cash_on_delivery_order_status_id');

                    $comment = $this->setting->get('component_payment_method_cash_on_delivery_instruction_' . $this->language->getCurrentId());
                ...
                $this->model_checkout_order->editOrderPaymentMethodText($this->customer->getId(), $this->session->get('customer_order_id'), $this->setting->get('component_payment_method_cash_on_delivery_instruction_' . $this->language->getCurrentId()));
