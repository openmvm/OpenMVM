...
class Checkout extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->model_checkout_order = new \App\Models\Marketplace\Checkout\Order_Model();
    }
    ...
    public function index()
    {
        ...
        $breadcrumbs[] = array(
            'text' => lang('Text.checkout'),
            'href' => $this->url->customerLink('marketplace/checkout/checkout', '', true),
            'active' => true,
        );

        $data['heading_title'] = lang('Heading.checkout');
        ...
        if ($this->customer->isLoggedIn()) {
            ...
            foreach ($sellers as $seller) {
                ...
                foreach ($shipping_methods as $shipping_method) {
                    $namespace = '\App\Models\Marketplace\Component\Shipping_Method\\' . $shipping_method['value'] . '_Model';
    ...
    public function payment_method()
    {
        if ($this->customer->isLoggedIn()) {
            ...
            foreach ($payment_methods as $payment_method) {
                $namespace = '\App\Models\Marketplace\Component\Payment_Method\\' . $payment_method['value'] . '_Model';
            ...
            foreach ($sellers as $seller) {
                ...
                foreach ($results as $result) {
                    if ($this->setting->get('component_order_total_' . strtolower($result['value']) . '_status')) {
                        $namespace = '\App\Models\Marketplace\Component\Order_Total\\' . $result['value'] . '_Model';
    ...
    public function confirm()
    {
        if ($this->customer->isLoggedIn()) {
            ...
            if (!$this->session->has('checkout_payment_address_id')) {
                $data['errors'][] = lang('Error.checkout_payment_address');
            } else {
            ...
            if (!$this->session->has('checkout_shipping_address_id')) {
                $data['errors'][] = lang('Error.checkout_shipping_address');
            } else {
            ...
            foreach ($sellers as $seller) {
                ...
                if (!$this->session->has('checkout_shipping_method_' . $seller['seller_id'])) {
                    $data['errors'][] = sprintf(lang('Error.checkout_shipping_method'), $seller['store_name']);
                } else {
            ...
            if (!$this->session->has('checkout_payment_method_code')) {
                $data['errors'][] = lang('Error.checkout_payment_method');
            } else {
                // Checkout payment method
                $order_data['payment_method_code'] = $this->session->get('checkout_payment_method_code');
                $order_data['payment_method_title'] = lang('Text.' . strtolower($this->session->get('checkout_payment_method_code')));
            }
            ...
            if (empty($data['errors'])) {
                ...
                $namespace = '\App\Controllers\Marketplace\Component\Payment_Method\\' . $checkout_payment_method_code;
                ...
                $data['message'] = sprintf(lang('Text.checkout_selected_payment_method'), lang('Text.' . strtolower($checkout_payment_method_code)));
