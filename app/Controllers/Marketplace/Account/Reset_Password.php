...
class Reset_Password extends \App\Controllers\BaseController
{
    ...
    {
        $this->model_account_reset_password = new \App\Models\Marketplace\Account\Reset_Password_Model();
    }
    ...
    public function index()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($this->validation->withRequest($this->request)->run()) {
                ...
                $this->session->set('success', lang('Success.reset_password'));
    ...
    public function get_form($data)
    {
        ...
        $breadcrumbs[] = array(
            'text' => lang('Text.reset_password'),
            'href' => $this->url->customerLink('marketplace/account/reset_password'),
            'active' => true,
        );

        $data['heading_title'] = lang('Heading.reset_password');
        ...
        $data['reset_password_instruction'] = sprintf(lang('Text.reset_password_instruction'), $this->setting->get('setting_marketplace_name'), $this->setting->get('setting_marketplace_name'));

        // Header
        $header_params = array(
            'title' => lang('Heading.reset_password'),
    ...
    public function confirm()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            $this->validation->setRule('passconf', lang('Entry.passconf'), 'required');
            ...
            if ($this->validation->withRequest($this->request)->run()) {
                ...
                if ($verify_token) {
                    ...
                    $this->session->set('success', lang('Success.reset_password_confirm'));
                } else {
                    $this->session->set('error', lang('Error.reset_password_confirm'));
                }
            } else {
                if ($this->validation->hasError('email')) {
                    $data['error_email'] = $this->validation->getError('email');
        ...
        $data['reset_password_confirm_instruction'] = lang('Text.reset_password_confirm_instruction');
