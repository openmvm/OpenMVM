...
class BackendTheme
{
  ...
  {
		// Get modules
		$directories = glob(ROOTPATH . 'modules/*/BackendTheme', GLOB_ONLYDIR);
    ...
    $settings = array();
  	...
  	foreach ($theme_settings as $theme_setting) {
  		...
  		if (is_file(ROOTPATH . 'modules/' . $theme_setting ['provider'] . '/BackendTheme/Controllers/BackEnd/Theme'. $theme_setting ['dir'] .'.php')) {
	  		$settings[] = array(
  ...
  {
		// Get modules
		$directories = glob(ROOTPATH . 'backend_themes/*/*', GLOB_ONLYDIR);
