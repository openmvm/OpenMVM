...
class Cod extends \App\Controllers\BaseController
{
	...
	public function index()
	{
		...
		// Data Libraries
		$cod_data['lang'] = $this->language;

		// Return view
		return $this->template->render('FrontendThemes', 'PaymentMethod\cod', $cod_data);
	}

	public function confirm() {
    $json = array();

    $order_status_id = $this->setting->get('payment_cod', 'payment_cod_order_status_id');

		if ($this->session->has('payment_method')) {
			$payment_method = $this->session->get('payment_method');

			if (!empty($payment_method['code']) && $payment_method['code'] == 'cod') {
