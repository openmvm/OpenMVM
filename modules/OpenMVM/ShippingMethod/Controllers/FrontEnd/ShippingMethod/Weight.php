...
class Weight extends \App\Controllers\BaseController
{
	...
	public function index()
	{
		...
		// Data Link
		$data['action'] = base_url('/account/store/shipping_methods/OpenMVM/weight/edit/' . $this->user->getToken());

		// Form
		if ($this->request->getMethod() === 'post') {
      // Query
      $query = $this->settingModel->editSettings('vendor_' . $this->user->getStoreId() . '_shipping_weight', $this->request->getPost());
	...
	public function getForm($data = array())
	{
		...
		$data['breadcrumbs'][] = array(
			'text' => lang('Heading.heading_weight', array(), $this->language->getFrontEndLocale()),
			'href' => '',
			'active' => true,
		);

		// Data form
		if ($this->request->getPost('vendor_' . $this->user->getStoreId() . '_shipping_weight_status')) {
			$data['vendor_' . $this->user->getStoreId() . '_shipping_weight_status'] = $this->request->getPost('vendor_' . $this->user->getStoreId() . '_shipping_weight_status');
		} else {
			$data['vendor_' . $this->user->getStoreId() . '_shipping_weight_status'] = $this->setting->get('vendor_' . $this->user->getStoreId() . '_shipping_weight', 'vendor_' . $this->user->getStoreId() . '_shipping_weight_status');
		}
		...
		$header_parameter = array(
			'title' => lang('Heading.heading_weight', array(), $this->language->getFrontEndLocale()),
		...
		if ($this->user->hasPermission()) {
			echo $this->template->render('FrontendThemes', 'ShippingMethod\ShippingMethod\weight', $data);
		} else {
