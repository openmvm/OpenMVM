...
class Latest extends \App\Controllers\BaseController
{
    ...
    $data['action'] = base_url($_SERVER['app.adminDir'] . '/frontend_widget/edit/OpenMVM/Store/latest/' . $this->request->uri->getSegment(7) . '/' . $this->administrator->getToken());
    	...
    	if ($this->administrator->hasPermission('modify','modules/OpenMVM/Store/Controllers/BackEnd/FrontendWidgets/Latest')) {
	      // Query
	...
	public function getForm($data = array())
	{
		...
		$data['breadcrumbs'][] = array(
			'text' => lang('Heading.heading_latest', array(), $this->language->getBackEndLocale()),
			'href' => '',
			'active' => true,
		);

		// Data Text
		$data['heading_title'] = lang('Heading.heading_latest', array(), $this->language->getBackEndLocale());
		$data['lead'] = lang('text.text_latest_lead', array(), $this->language->getBackEndLocale());
		...
		$header_parameter = array(
			'title' => lang('Heading.heading_latest', array(), $this->language->getBackEndLocale()),
		);
		...
		// Echo view
		if ($this->administrator->hasPermission('access','modules/OpenMVM/Store/Controllers/BackEnd/FrontendWidgets/Latest')) {
			echo $this->template->render('BackendThemes', 'Store\FrontendWidgets\latest', $data);
		} else {
