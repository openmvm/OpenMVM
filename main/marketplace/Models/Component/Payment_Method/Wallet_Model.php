...
class Wallet_Model extends Model
{
    ...
    public function method($customer_address_id = 0, $total)
    {
        ...
        if (!$this->setting->get('component_payment_method_wallet_status')) {
            return false;
        }

        if ($this->setting->get('component_payment_method_wallet_amount') > 0 && $this->setting->get('component_payment_method_wallet_amount') > $total) {
            return false;
        }

        if ($total > $this->customer->getWalletBalance()) {
            return false;
        }
        ...
        $zone_to_geo_zone_builder->where('geo_zone_id', $this->setting->get('component_payment_method_wallet_geo_zone_id'));
        ...
        $method_data = [
            'code' => 'Wallet',
            'name' => lang('Text.wallet', [], $this->language->getCurrentCode()) . ' ( ' . lang('Text.balance', [], $this->language->getCurrentCode()) . ': ' . $this->currency->format($this->customer->getWalletBalance(), $this->currency->getCurrentCode()) . ' ; ' . lang('Text.total_order_amount', [], $this->language->getCurrentCode()) . ': ' . $this->currency->format($total, $this->currency->getCurrentCode()) . ' )',
            'sort_order' => $this->setting->get('component_payment_method_wallet_sort_order'),
        ];
