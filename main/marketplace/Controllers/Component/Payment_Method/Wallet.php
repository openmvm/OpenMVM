...
class Wallet extends \App\Controllers\BaseController
{
    ...
    {
        $data['instruction'] = nl2br($this->setting->get('component_payment_method_wallet_instruction_' . $this->language->getCurrentId()));

        if (!empty($seller_id)) {
            $data['confirm'] = $this->url->customerLink('marketplace/component/payment_method/wallet/confirm', ['seller_id' => $seller_id], true);
        } else {
            $data['confirm'] = $this->url->customerLink('marketplace/component/payment_method/wallet/confirm', '', true);
        }
    ...
    public function confirm()
    {
        ...
        if ($this->session->has('checkout_payment_method_code') && $this->session->get('checkout_payment_method_code') == 'Wallet') {
            $error = false;
            ...
            if (!$error) {
                ...
                foreach ($sellers as $seller) {
                    ...
                    // Subtract wallet balance for customer
                    $wallet_description = [];

                    foreach ($languages as $language) {
                        $wallet_description[$language['language_id']] = lang('Text.order_payment_customer', ['order_id' => $this->session->get('checkout_order_id'), 'store_name' => $seller['store_name']], $this->language->getCurrentCode());
                    }
                    ...
                    $this->model_account_wallet->addWallet($this->customer->getId(), $wallet_input_data);

                    // Add order status history
                    $order_id = $this->session->get('checkout_order_id');

                    $order_status_id = $this->setting->get('component_payment_method_wallet_order_status_id');

                    $comment = [];

                    foreach ($languages as $language) {
                        if (!empty($this->setting->get('component_payment_method_wallet_instruction_' . $language['language_id']))) {
                            $comment[$language['language_id']] = $this->setting->get('component_payment_method_wallet_instruction_' . $language['language_id']);
                        }
                    }
                ...
                $this->model_checkout_order->editOrderPaymentMethodText($this->customer->getId(), $this->session->get('customer_order_id'), nl2br($this->setting->get('component_payment_method_wallet_instruction_' . $this->language->getCurrentId())));
